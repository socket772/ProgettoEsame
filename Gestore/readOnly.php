<html>
    <style><?php include './stili/css/bootstrap-italia.min.css'; ?></style>
    <body>
        <h1 class="display-1">Scegli la tabella da visualizzare</h1>
        <br><br>
        <button class="btn btn-primary btn-block" onclick="window.location.href='./Fornitori/visualizzaFornitori.php';">Visualizza fornitori</button>
        <br><br>
        <button class="btn btn-primary btn-block" onclick="window.location.href='./Inventario/visualizzaInventario.php';">Visualizza invetario</button>
        <br><br>
        <button class="btn btn-primary btn-block" onclick="window.location.href='./Ordini/visualizzaOrdini.php';">Visualizza ordini</button>
        <br><br>
        <button class="btn btn-primary btn-block" onclick="window.location.href='./Ordini/visualizzaStoricoOrdini.php';">Visualizza ordini</button>

        <script src="./stili/js/bootstrap-italia.bundle.min.js"></script>
    </body>
</html>